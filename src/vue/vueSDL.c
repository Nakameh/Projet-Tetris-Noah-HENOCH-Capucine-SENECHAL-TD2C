...
void createWindowSDL(Partie * p) {
    windowManager * wm = windowManager_init("Tetris", p->plateauDeJeu->nbColonne * 32 + 220,p->plateauDeJeu->nbLigne * 32 + 100);
...
void updateWindowSDL(windowManager * wm, Partie * p) {
    ...
    dessineRectangle(wm, 0, 0, p->plateauDeJeu->nbColonne * 32 + 220, p->plateauDeJeu->nbLigne * 32 + 100, colors[0]);
    dessineLigneBlanche(wm, 9, 99, 8, p->plateauDeJeu->nbLigne * 32 + 90);
    dessineLigneBlanche(wm, 9, 99, p->plateauDeJeu->nbColonne * 32 + 210, 99);
    dessineLigneBlanche(wm, p->plateauDeJeu->nbColonne * 32 + 210, 99, p->plateauDeJeu->nbColonne * 32 + 210, p->plateauDeJeu->nbLigne * 32 + 90);
    dessineLigneBlanche(wm, 9, p->plateauDeJeu->nbLigne * 32 + 90, p->plateauDeJeu->nbColonne * 32 + 210, p->plateauDeJeu->nbLigne * 32 + 90);
    ...
    SDL_Rect textRect2 = {10, 40, textSurface2->w, textSurface2->h};
    SDL_Rect textRect3 = {10, 80, textSurface3->w, textSurface3->h};
    ...
    for (int ligne = 0; ligne < p->plateauDeJeu->nbLigne; ligne ++) {
        for (int colonne = 0; colonne < p->plateauDeJeu->nbColonne; colonne++) {
            ...
            if (c->occupe) {
                dessineRectangle(wm, colonne * 32 + 10, ligne * 32 + 100, 32, 32, colors[c->color]);
            }
        }
    }
